...
export default function Timeline({ timelineData }) {
    ...
    return (
        <div className={`relative overflow-hidden w-[640px] transition-all duration-500`} style={{ height: isOpen ? `${expandedHeight}px` : `${collapsedHeight}px` }}>
                    ...
                    {timelineData.slice(1, -1).map((item, index) => (
                        <div key={index} className={`w-40 ${index % 2 === 0 ? 'right-[320px]' : 'left-[320px]'} absolute inline-flex flex-col justify-start items-center gap-2.5`} style={{ top: `${300 + index * 300}px` }}>
                            ...
                            <div className={`w-[300px] h-32 ${index % 2 === 0 ? 'left-[-160px]' : 'left-[20px]'} top-[60px] absolute text-${index % 2 === 0 ? 'right' : 'left'} justify-start text-black text-xl font-normal font-['Belleza']`}>
                                {item.text}</div>
                            <img className={`w-48 h-48 ${index % 2 === 0 ? 'left-[260px]' : 'left-[-260px]'} top-[-40px] absolute origin-top-left rotate-[${index % 2 === 0 ? '-13deg' : '15deg'}] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] border-[6px] border-white`} 
                                src={item.image} />
                        </div>
                    ))}
                </CollapsibleContent>
            ...
            <div className={`left-[280px] top-[270px] absolute`} style={{ background: isOpen ? `` : 'white' }}>{isOpen ? '' : 'See More' }</div>
            ...
            {/* Blur Overlay */}
            {!isOpen && (
                <div className="absolute top-[370px] w-[640px] h-[300px] backdrop-blur-sm"
                    style={{
