...
export async function GET(req) {
    ...
    const ratingRes = await fetch(`https://api-cdn.yotpo.com/products/${appKey}/${externalId}/bottomline`);

    // Check if the response is ok
    if (!ratingRes.ok) {
        const err = await ratingRes.text();
        return new Response(JSON.stringify({ message: 'Failed to fetch star rating data', err }), { status: 500 });
    }
