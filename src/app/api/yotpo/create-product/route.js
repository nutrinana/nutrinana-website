...
export async function GET() {
    ...
    if (createRes.ok) {
        ...
        if (alreadyExists) {
            ...
            const lookupRes = await fetch(
                `https://api.yotpo.com/core/v3/stores/${storeId}/products?external_id=${product.external_id}`,
                {
                    headers: {
                        "Content-Type": "application/json",
                        "X-Yotpo-Token": access_token,
            ...
            if (!yotpoId) {
                return new Response(
                    JSON.stringify({ message: "Yotpo ID not found in lookup response" }),
                    {
                        status: 500,
                    }
                );
            ...
            const updateRes = await fetch(
                `https://api.yotpo.com/core/v3/stores/${storeId}/products/${yotpoId}`,
                {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json",
    ...
    return new Response(JSON.stringify({ product: productResponse, existingProduct }), {
        status: 200,
    });
}
