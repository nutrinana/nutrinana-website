...
export async function GET(request) {
    try {
        ...
        if (!apiKey || !serial) {
            return jsonError('Missing COOKIEBOT_API_KEY or NEXT_PUBLIC_COOKIEBOT_DOMAIN_ID env variables.', 500, {
                hint: 'Set both env vars in your environment and redeploy.',
            });
        }

        if (!domain) {
            return jsonError('Missing domain. Provide ?domain=www.example.com or set NEXT_PUBLIC_SITE_DOMAIN.', 400);
        }
